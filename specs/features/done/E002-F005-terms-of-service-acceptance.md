...
- `/Users/young/Nextcloud/dev/Itervel/tests/Browser/Auth/TermsOfServiceTest.php` -- Pest browser tests: smoke tests for the terms-of-service page, dark mode spot check, and registration flow tests with terms acceptance using `data-test` selectors.
...
- Browser Test Developer
    - Name: tos-browser-test-dev
    - Role: Writes Pest browser tests (smoke tests, dark mode check, interactive flow tests) for the terms-of-service page and registration flow with terms acceptance
    - Agent Type: coder
    - Resume: false
...
- Create `/Users/young/Nextcloud/dev/Itervel/resources/js/pages/terms-of-service.tsx`:
    ...
    - Add `data-test="tos-content"` attribute on the main content container `<div>`
...
- Add `data-test` attributes to interactive elements for browser testing:
    - `data-test="tos-link"` on the Terms of Service `<a>` tag in the registration form
    - `data-test="terms-checkbox"` on the terms checkbox element in the registration form
    - `data-test="register-submit"` on the registration submit button (if not already present from E002-F001)
...
- **Assigned To**: tos-browser-test-dev
- **Agent Type**: coder
- **Parallel**: false
- Read the frontend pages to understand the markup and `data-test` attributes:
    - `/Users/young/Nextcloud/dev/Itervel/resources/js/pages/terms-of-service.tsx`
    - `/Users/young/Nextcloud/dev/Itervel/resources/js/pages/auth/register.tsx`
- Create `tests/Browser/Auth/TermsOfServiceTest.php` using `php artisan make:test --pest Browser/Auth/TermsOfServiceTest --no-interaction`
- Write a smoke test for the terms-of-service page:
    - Visit `/terms-of-service` as a guest (no authentication required)
    - Assert successful response and no JavaScript errors
    - Assert the content container `[data-test="tos-content"]` is visible
- Write a dark mode spot check for the terms-of-service page:
    - Visit `/terms-of-service`, switch to dark color scheme with `colorScheme('dark')`, assert no JavaScript errors
- Write an interactive flow test for registration with terms acceptance:
    - Visit the registration page
    - Fill in name, email, password, and password confirmation fields
    - Click `[data-test="terms-checkbox"]` to accept terms
    - Click `[data-test="register-submit"]` to submit
    - Assert the user is redirected to the dashboard (or email verification page)
    - Assert no JavaScript errors
- Write a test verifying the ToS link opens in a new tab:
    - Visit the registration page
    - Assert `[data-test="tos-link"]` has `target="_blank"` attribute
    - Assert no JavaScript errors
- Run browser tests: `php artisan test tests/Browser/Auth/TermsOfServiceTest.php --compact`
- Run `vendor/bin/pint --dirty` to fix any formatting issues

### 5. Validate Complete Implementation

- **Task ID**: validate-all
- **Depends On**: backend-migration-models-route, frontend-tos-page-and-form, write-tos-tests, write-browser-tests
...
- Run browser tests: `php artisan test tests/Browser/Auth/TermsOfServiceTest.php --compact`
- Verify `data-test` attributes exist on interactive elements in `terms-of-service.tsx` and `register.tsx`
...
- Dark mode spot check passes for the terms-of-service page
- Core registration flow with terms acceptance passes browser tests using `data-test` selectors
...
# Run browser tests
php artisan test tests/Browser/Auth/TermsOfServiceTest.php --compact
```
